...
2. 地址可以只填 `主机:端口`（比如 `192.0.2.10:8896`），程序会自己补上协议；局域网里也能自动发现服务器。
...
- **播放行为**：断点续播、自动连播、开始播放后自动全屏、片头片尾
- **字幕**：语言优先级和字幕外观
- **视频输出 / 音频输出**：渲染方式（集成模式／独占模式，独占模式下可一边挂着片子一边翻媒体库，视频窗内带 uosc 进度条与轨道菜单）、硬件解码、输出设备、音频独占、功放直通
