...
**播放器** — 内置 libmpv，随程序打包，不用另外装 mpv。进度、音量、字幕、音轨、章节、倍速、全屏、窗口置顶都在播放器里直接调。自动跳过片头片尾，自动连播下一集（跨季也接），断点续播，进度自动回传服务器。鼠标 2 秒不动自动隐藏。
...
这是一个个人自用的客户端，纯 AI 开发。`libmpv-2.dll`（117 MB）不入库，构建前需要放到仓库根目录。架构、构建、发布和自检的细节见 [docs/开发与验证.md](docs/开发与验证.md)。
