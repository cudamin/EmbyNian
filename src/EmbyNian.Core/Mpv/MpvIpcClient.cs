...
public sealed class MpvIpcClient : IAsyncDisposable
{
    ...
    /// <summary>
    /// A property as mpv's own formatted text. <c>get_property_string</c> rather than
    /// <c>get_property</c>, so properties whose JSON form is a number or an object still arrive as
    /// the string mpv would print — which is what a statistics readout wants to show.
    /// </summary>
    public async Task<string?> GetTextAsync(string name, CancellationToken cancellationToken = default) =>
        (await RequestAsync(cancellationToken, "get_property_string", name).ConfigureAwait(false))?.AsString();
