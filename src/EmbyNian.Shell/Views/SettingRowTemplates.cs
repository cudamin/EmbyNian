...
/// <para>
/// The settings page is around sixty rows of nine shapes, so the shapes are written once as templates and
/// the rows are data. This is what joins the two. The alternative — a <c>Visibility</c> per shape on every
/// row, with all nine controls built for each — realises nine times the controls and leaves nine bindings
/// live on every row, all but one of them pointed at properties its row does not have.
/// </para>
/// <para>
...
public sealed partial class SettingRowTemplates : DataTemplateSelector
{
    ...
    public DataTemplate? ConfigEditor { get; set; }
