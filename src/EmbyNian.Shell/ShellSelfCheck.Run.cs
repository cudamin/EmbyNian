    ...
    private static void Run(HostWindow window, ShellPage shell, IServiceProvider services, StartupOptions options)
    {
        ...
        report.AppendLine($"[信息] 页面形状 — {width}×{height} = {browseShape:0.000}:1"
            ...
            + (Math.Abs(browseShape - HomeCarousel.WindowAspect) < 0.01
                ...
                : $"不是 {HomeCarousel.WindowAspectLabel}（开窗那一档才是），轮播照旧铺满整条带、裁的是上下两头"));
        ...
        else
        {
            ...
            // 「把媒体库的列表也添加到主页之中，新增页里拖拽决定这些列表的顺序，勾选显示或者不勾选取消显示」：
            // 屏上那几排要和设置里那份版面一字不差 —— 次序一样、勾掉的那几排真没排。少一排、次序错一位，在截图里
            // 都只是「主页看起来有点不一样」。
            Check("主页版面", home.LayoutOk, home.Layout);

            // 屏上的牌子（2026-09-14「继续观看点了没反应」之后补的一关）：主页版面量的是数据（该带 › 的排都
            // 带），货架牌子进库量的是独立的牌子控件（Offer 一开、门就开）——中间隔着一格从来没量过：真树上
            // 那一块牌子，大于号真的露出来了吗、点击面真的铺开了吗、这一排真的挂到牌子上了吗。那次断的就是
            // 最后一问：数据全对、控件全对，模板把这一排挂错的通道在真树上没有人看。读数在 stage 0 快照
            // （主页还在框里的时候），这里只判。
            if (_headOnScreen is { } headRead) Check("屏上的牌子", headRead.Ok, headRead.Detail);

            // 「去掉首页封面轮播图的黑边」（2026-09-11）：剧照铺满整条带 —— 元素就是整格、图按自己的比例放大到
            // 盖住、多出来的一截对半裁掉。屏幕上「一边露出一条底色」长得像「照片没占满」，不像一个错，所以这一条
            // 量的是元素和格子一样大、四边不偏，再把「这张图进这条带裁掉多少」算出来报给人看。从前的名字是
            // 「主页大图不裁切」—— 那一版要求图整张画出来、两边留底色，正是他这次要去掉的东西。
            Check("主页大图铺满", home.PictureOk, home.Picture);

            // 「占满窗口的上半部分（包括窗口标题），下方使用渐变衔接」（2026-09-11）：量那条带在窗口里的位置，
            // 左沿、上沿、右沿都该顶到窗口的边（上沿连标题栏那 32 也吃掉，那一行浮在剧照上）。谁把主页标记里那
            // 条负的上边距删掉、或者给它补回左右留白，这一条当场红。从前这一读叫「轮播卡片留边」，判的正好相反
            // （2026-09-10 到 09-11 之间的「框成卡片」那一版：四条边都让出 24）。
            Check("主页大图通栏", home.BleedOk, home.Bleed);

            // 通栏之后这一读量的是：带高就是 HomeCarousel.Height 按它自己的宽算出来的那个数（带宽＝整个页宽），
            // 整块放得进第一屏，横排接在它下沿之后。这条在真实 XAML 树上量。
            if (home.FoldOk is { } foldOk) Check("主页首屏", foldOk, home.Fold);
            else report.AppendLine($"[信息] 主页首屏 — {home.Fold}");

            // 剧照铺到窗口的顶边之后标题栏那一行浮在图上，那三颗系统窗口按钮的墨必须换成固定的浅墨（见
            // ReadInk）—— 主题那支深墨画在剧照上就是几颗看不见的按钮。联动删了、或者换错了方向，这一条红。
            if (_ink is { } ink) Check("标题栏墨色浮图", ink.Ok, ink.Detail);

            // 页眉墨色只报不判：通栏之后这一页的页眉（面包屑那一行）也在剧照上，所以 OnScrim 那一档该开着。
            report.AppendLine($"[信息] 页眉墨色 — {home.Slate}");
        ...
        // 出来。同样和数据无关 —— 「点进去落到哪一页」由 HomeLayout.TargetOf + HomeViewModel.OpenShelf 一层层
        // 接起来，自检没有服务器，一排都没有，量不到那一头（能点到的那一头有别的关口看着：主页版面那一行报
        // 每一排带不带 ›）。
