...
internal static partial class ShellSelfCheck
{
    private static void Run(HostWindow window, ShellPage shell, IServiceProvider services, StartupOptions options)
    {
        ...
        else
        {
            ...
            // 「轮播的海报能保持16:9」：剧照整张画出来，不裁上下。屏幕上「裁掉了三成半」只是一张构图不太对的
            // 图，没人指得出这是个错，所以这一条量的是画出来那张图和原图的形状对不对得上。
            Check("主页大图不裁切", home.PictureOk, home.Picture);

            // 「弄个框把轮播图框起来」（2026-09-10 下半天，真意：不占满上半页、和窗口四边留间隔）：量轮播卡片
            // 在窗口里的位置，四条边都该让出来 —— 谁把它改回铺满整宽贴到窗口边，这一条当场红。从
            // 前这一读叫「主页大图贴边」，判的正好相反（上沿 0、右沿吃满窗口宽）。
            Check("轮播卡片留边", home.BleedOk, home.Bleed);

            // 轮播 2026-09-10 框成卡片之后这一读量的是：卡片高度就是 HomeCarousel.Height 按它自己的宽算出来
            // 的那个数（带宽比页宽窄 48），整块放得进第一屏，横排接在它下沿之后。这条在真实 XAML 树上量。
            if (home.FoldOk is { } foldOk) Check("主页首屏", foldOk, home.Fold);
            else report.AppendLine($"[信息] 主页首屏 — {home.Fold}");

            // 轮播框成卡片之后标题栏底下永远是页面底色，那三颗系统窗口按钮的墨必须一直是主题那支（见
            // ReadInk）—— 从前的联动（图铺上去换浅墨）没删干净的话这一条红。
            if (_ink is { } ink) Check("标题栏墨色固定", ink.Ok, ink.Detail);

            // 页眉墨色只报不判：轮播成了卡片，页眉站在页面自己的纸上，OnScrim 那一档不该再开。
