-- 播放统计（mpv 内置 stats.lua 的简体中文覆盖版）
--
-- 本文件是 mpv 内置脚本 stats.lua 的“中文覆盖版”：
--   * 统计项（页面 / 字段 / 数量 / 顺序 / 计算逻辑）与参考项目
--     https://github.com/dyphire/mpv-config 所用的上游 stats.lua 完全一致，
--     仅把展示文案译为简体中文；
--   * 放在 portable_config/scripts/stats.lua 即可覆盖内置脚本，
--     并在 mpv.conf 中以 load-stats-overlay=no 关闭内置版，避免两者重复注册。
--
-- 上游原说明：
-- Display some stats.
--
...
local o = {
    ...
    ass_formatting = true,
    persistent_overlay = true,       -- 独立 OSD 层，不被音量、跳转提示覆盖
...
local frame_labels = {fresh = "新帧（Fresh）", redraw = "重绘（Redraw）"}

local function append_perfdata(header, s, dedicated_page)
    local vo_p = mp.get_property_native("vo-passes")
    if not vo_p or next(vo_p) == nil then
        if dedicated_page then
            s[#s+1] = o.nl .. "尚无 GPU 帧计时数据，请等待视频输出就绪。"
            s[#s+1] = o.nl .. "部分视频输出不提供此项数据。"
        end
        return
    end
    ...
    h[#h+1] = format("%s%s%s%s%s%s%s%s",
                     ...
                     bold("帧计时（Frame Timings）："), o.prefix_sep, font_small,
                     "(最近 / 平均 / 峰值，微秒 μs)", font_normal,
                     dedicated_page and "  PageUp/PageDown 翻页" or "")
    ...
    for _,frame in ipairs(sorted_keys(vo_p)) do  -- ensure fixed display order
        ...
        if dedicated_page then
            s[#s+1] = format("%s%s%s:", o.nl, o.indent,
                             bold(frame_labels[frame] or frame))
...
local function add_file(s, print_cache, print_tags)
    ...
    if demuxer_cache + demuxer_secs > 0 then
        append(s, utils.format_bytes_humanized(demuxer_cache), {prefix="缓存总量："})
...
local function append_resolution(s, r, prefix, w_prop, h_prop, video_res)
    ...
    if append(s, r[w_prop], {prefix=prefix}) then
        append(s, r[h_prop], {prefix="×", nl="", indent=" ", prefix_sep=" ",
        ...
        if r["crop-w"] and (not video_res or
            ...
            append(s, format("[x: %d, y: %d, 宽: %d, 高: %d]",
...
local function append_img_params(s, r, ro)
    ...
    append(s, r["colormatrix"], {prefix="颜色矩阵："})
    if r["prim-red-x"] or r["prim-red-y"] or
       ...
       r["prim-white-x"] or r["prim-white-y"] then
        ...
        append(s, r["primaries"], {prefix="为", nl="", indent=" ", prefix_sep=" ",
                                   no_prefix_markup=true})
    else
...
local function add_video(s)
    ...
    if o.show_frame_info then
        ...
        local timecodes = {
            ...
            ["estimated-smpte-timecode"] = "估算 SMPTE",
        }
...
local function add_audio(s)
    ...
    local ao_mute = mp.get_property_native("ao-mute") and "（已静音）" or ""
    append_property(s, "ao-volume", {prefix="输出音量：", suffix="%" .. ao_mute,
                                     nl=dev and "" or o.nl,
...
local function finalize_page(header, content, apply_scroll)
    ...
    if apply_scroll then
        ...
        local max_content_lines = (o.use_ass
            and max(4, math.floor(288 / (font_size * 1.45)) - 5)
            or term_height - 2) - #header
...
local function default_stats()
    ...
    eval_ass_formatting()
    add_header(stats)
    stats[#stats+1] = bold("播放统计") .. "  ·  再点“统计”查看着色器统计" .. o.nl
...
local function vo_stats()
    ...
    eval_ass_formatting()
    add_header(header)
    header[#header+1] = bold("着色器统计") .. "  ·  再点“统计”关闭" .. o.nl
...
local function add_track(c, t, i)
    ...
    local track_types = {video = "视频", audio = "音频", sub = "字幕"}
    local type = t.image and "图像" or (track_types[t["type"]] or t["type"])
    ...
    local flags = {"default", "forced", "dependent", "visual-impaired",
                   ...
                   "albumart", "external"}
    local flag_names = {["default"] = "默认", ["forced"] = "强制",
                        ["dependent"] = "依赖", ["visual-impaired"] = "视障",
                        ["hearing-impaired"] = "听障", ["original"] = "原始",
                        ["commentary"] = "评论", ["image"] = "图像",
    ...
    if t["dolby-vision-profile"] or t["dolby-vision-level"] then
        ...
        append(c, t["dolby-vision-profile"], {prefix="配置：", nl="", indent=""})
        append(c, t["dolby-vision-level"], {prefix="等级：", nl="",
...
local function cache_stats()
    ...
    if seek_ts ~= nil then
        state = "定位中（至 " .. mp.format_time(seek_ts) .. "）"
    ...
    append(stats, "", {prefix="可定位区间：", nl=o.nl .. o.nl, indent=""})
...
local function cycle_stats()
    if not display_timer:is_enabled() then
        curr_page = o.key_page_1
        process_key_binding(false)
    elseif curr_page == o.key_page_1 then
        if display_timer.oneshot then
            -- 上一下是「一次性」（i 键）留下的面板：先收干净，再以常驻方式开着色器页。
            stop_stats()
            curr_page = o.key_page_2
            process_key_binding(false)
        else
            curr_page = o.key_page_2
            reset_scroll_offsets()
...
mp.add_key_binding(nil, "cycle-stats", cycle_stats, {repeatable=false})

-- 换源时不把上一集的统计与滚动位置带进新片。
