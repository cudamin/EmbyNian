...
**最新一件是「详情页第一屏黑边统一，阈值跟着显示器走」，见下面第一件活（2026-09-05，四道闸门全绿，未提交）。上一件是「点了第二块屏上的应用之后鼠标不再自动隐藏」。再上一件是「详情页第一屏黑边统一 280→460」——做了、验了，用户一句「改回去」当天整个撤掉（这一批是它按新规格重做的版本）。**

**⚠️ 这一整批（含三轮代码审查那些条，加最后两件改的几份文档）到本行写下时还在工作树里，没有提交、没有推到 origin。** 用户没喊提交，本项目的规矩是「只有他明确说了才提交」。下面几段里凡写着「随本条说明一起提交并推到 origin」的，说的是打算，不是已经发生的事 —— 接手的人先看 `git status`，别照文档判断。
